...
/// Get or create an indirect function pointer that references the given function.
///
/// Similar to \ref IRGenModule::getAddrOfLLVMVariableOrGOTEquivalent, but this one
/// always creates a new trampoline constant pointing to the function even though
/// it can be referenced directly.
/// The indirect function pointer is used to create a relative indirect function pointer
/// for harvard architecture targets.
llvm::Constant *
IRGenModule::getAddrOfIndirectFunctionPointer(llvm::Function *fn) {
  auto &entry = IndirectRelativeFunctionPointees[fn];
  if (entry) {
    return IndirectRelativeFunctionPointees[fn];
  }
  auto *pointer = new llvm::GlobalVariable(Module, fn->getType(),
                                          /*isConstant=*/true,
