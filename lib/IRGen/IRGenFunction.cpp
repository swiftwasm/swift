...
llvm::Value *
IRGenFunction::emitLoadOfRelativeFunctionPointer(Address addr, bool isFar,
                                                 llvm::PointerType *expectedType,
                                                 const llvm::Twine &name) {
  if (IGM.getOptions().IndirectRelativeFunctionPointer) {
    llvm::Value *value = emitLoadOfRelativePointer(Address(addr.getAddress(), IGM.getPointerAlignment()), isFar, expectedType->getPointerTo(), name);
